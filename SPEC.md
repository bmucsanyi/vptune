...
- `vp.loss.*` is a scalar loss that also exposes its output-space Hessian $H_\ell$. `vp.loss.softmax_cross_entropy(output="logits", labels="labels", mask=None, reduction="token_mean", denominator="num_tokens")` has the per-token block $\operatorname{diag}(p_t)-p_t p_t^\top$, and the operator-level $H_\ell$ is the masked, normalized sum $\frac{1}{N}\sum_{m_t=1}(\operatorname{diag}(p_t)-p_t p_t^\top)$ with $N=\sum_t m_t$ and a zero block at masked positions. `vp.loss.kl(output="logits", target="target", mask=None, reduction="token_mean", denominator="num_tokens")` reads a target-probability tensor with the same shape as logits, requires finite nonnegative entries, uses per-row target mass to scale the softmax block, and treats zero target entries as zero contribution to the KL value. Its Hessian block is $(\sum_c q_{tc})(\operatorname{diag}(p_t)-p_t p_t^\top)$ before the same mask and reduction scaling as CE. `vp.loss.mse(output="logits", target="target", mask=None, reduction="mean", denominator="num_elements")` reads a target tensor with the same shape as the output, uses squared-error value terms, and has diagonal output-space Hessian $2I$ before mask and reduction scaling; for `token_mean`, its denominator must be `num_tokens`, otherwise its denominator must be `num_elements`. `vp.loss.from_scalar(fn, output=..., version=...)` takes $H_\ell$ from autodiff, PSD-checked at the probe. `vp.loss.declared_psd(output=..., factors=...)` treats `factors` as a finite dense matrix $L$ over the flattened output and defines $H_\ell=LL^\top$, with the first factor dimension equal to the flattened output width. `vp.loss.declared_psd_matrix_free(output=..., matvec=..., version=...)` treats `matvec(output, vector)` as the output-space Hessian product, requires a finite same-shaped result for every deterministic basis vector of the flattened output, constructs the dense output-space Hessian columns from those products, requires symmetry, and PSD-checks the dense Hessian by a Lanczos eigenvalue estimate with a residual bound. It is admitted only when $\theta_1-\|r_1\|\ge-\tau$ for the smallest Ritz value $\theta_1$ and its residual norm $\|r_1\|$, or refused. `reduction` is `sum`, `mean`, `token_mean`, or a named adapter reduction; `denominator` aligns with the likelihood denominator so CE-GGN and exact Fisher coincide.
...
- `vp.metric.*` is a typed metric: `vp.metric.dense(matrix=...)`, `vp.metric.diagonal(diag=...)`, `vp.metric.block_diagonal(blocks=...)`, `vp.metric.kfac(factors=...)`, `vp.metric.ekfac(eigvecs_a=..., eigvecs_g=..., corrected_eigenvalues=...)`, `vp.metric.low_rank(factor=..., diagonal=...)`, `vp.metric.ggn_derived(factors=...)`, and `vp.metric.matrix_free(operator=...)` where `operator` is a sibling product, an `Operator` carrying a `name`. The metric records the product name so dependency and cohort machinery reach it, and an anonymous operator is rejected; its operator must be an admitted PSD curvature (a GGN or Fisher), inverted through conjugate gradient with positive damping. It replaces the opaque `representation: Mapping[str, Any]`.
- `vp.samples.*` declares a sampled-Fisher sample source: `vp.samples.fixed_seed(seed, count, sampling_bound=None)` or `vp.samples.table(table=..., identity=..., sampling_bound=None)`. `sampling_bound=None` means exact-Fisher comparison rows are rejected for that source. A declared bound is one of three closed forms: `{"kind": "abs_or_rel", "max_abs_diff": ..., "max_rel_diff": ..., "norm_floor": ...}`; `{"kind": "matrix_bernstein", "failure_probability": ..., "norm_floor": ...}`; or `{"kind": "hutchinson_relative_variance", "failure_probability": ..., "norm_floor": ...}`. `failure_probability` is a float in `(0, 1)`, and `norm_floor`, `max_abs_diff`, and `max_rel_diff` are finite nonnegative floats.
- `vp.damping.*` is the typed damping for `inverse_metric_vp`, `inverse_metric_inner_vp`, and `inverse_sqrt_metric_vp`: `vp.damping.scalar(lam)`, `vp.damping.per_group(values)` keyed by parameter-surface block, `vp.damping.kfac_pi(lam, policy="trace_norm" | "equal")` for the Martens-Grosse factor split, and `vp.damping.eigenvalue_floor(lam)` for EKFAC. KFAC damping is declared only here, never as a metric-owned field; any KFAC metric-owned damping declaration is rejected. `kfac_pi` requires a `vp.metric.kfac` metric (it needs the separate $A$ and $G$ factor norms, which EKFAC discards); `eigenvalue_floor` requires `vp.metric.ekfac` and adds $\lambda$ to the corrected eigenvalues; admission rejects the other pairings.
...
- Sample source fields: the sample source kind (fixed_seed, table), the seed when fixed_seed, the sample count, the table identity when table, and the full sampling-bound declaration when exact-Fisher comparison is enabled.
...
`vp.sampled_fisher_vp(model, likelihood, samples=...)` declares $\hat F_S v = \frac{1}{nS}\sum_{i,s} g_{is}(g_{is}^\top v)$ for the sample source the `samples` argument declares, called as `sampled_fisher_vp(batch, vector)`. It is a separate operator family from exact FisherVP. The sample source carries the fixed sample table or the fixed seed and sample count $S$, and the exact-Fisher comparison formula used when a row enables exact-Fisher comparison. That formula is a declared bound: either an explicit absolute-or-relative numeric bound, a matrix-Bernstein bound computed from the sampled score-gradient contribution moments and declared failure probability, or a Hutchinson relative-variance bound computed from the sampled score-gradient contribution moments and declared failure probability. A free-form bound is refused at admission.
...
- `batch.per_example_block_size`: a finite positive integer domain.
...
- `inverse_metric.preconditioner=block_diagonal` requires block-diagonal blocks or KFAC factors. `inverse_metric.preconditioner=factorized_metric` requires diagonal, KFAC, EKFAC, low-rank, or GGN-derived factors. `inverse_metric.preconditioner=matrix_free` requires `inverse_metric.preconditioner_product` naming a sibling product whose operator is an admitted matrix-free preconditioner; the named product must have selected rows before this row's reference check. `inverse_metric.preconditioner_product` is a required companion setting for the `matrix_free` value and is read by the `inverse_metric.preconditioner` admission and lowering rules, not an independent sweep axis. `inverse_metric.block_schedule` requires block-diagonal blocks or KFAC factors. `inverse_metric.multi_rhs=block` applies the solve to a stacked right-hand side, and joins `metric_vp`/`inverse_metric_vp` to the vectorization applicability.
...
- `per_example_gradient.accumulation=blockwise_stacked` requires `batch.per_example_block_size` to size the example-blocks; `stacked_leading_axis` does not read it. The returned tree is identical for both; only peak memory differs.
...
- `inverse_metric.preconditioner=matrix_free` applies the sibling product named by `inverse_metric.preconditioner_product` as the preconditioner inside the iterative solve.
...
- `per_example_gradient.grad_path` lowers like the empirical-Fisher gradient paths but writes the stacked per-example gradients to a leading axis instead of reducing them. `per_example_gradient.accumulation=stacked_leading_axis` materializes all $n$ gradients then stacks them into the leading-axis tree; `blockwise_stacked` computes them in example-blocks of `batch.per_example_block_size`, writing each block into the same preallocated output, so the returned tree is identical and only peak memory differs.
...
- Per-example gradient tests cover `vp.per_example_gradient` against a loop reference and confirm `empirical_fisher_vp` equals its outer-product reduction; a `blockwise_stacked` row with a declared `batch.per_example_block_size` returns the tree identical to the `stacked_leading_axis` row.
- Typed damping tests cover `vp.damping.scalar`, `vp.damping.per_group`, and `vp.damping.kfac_pi`; KFAC metric tests reject metric-owned damping declarations.
- Solver-tolerance tests cover `tol` on an iterative inverse row (`conjugate_gradient`) setting both the CG stopping criterion and the accepted inverse residual, and `tol` on a matrix-free inverse-square-root Lanczos row setting the accepted Lanczos residual estimate.
