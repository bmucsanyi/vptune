...
from vptune import fisher, ggn, metrics, runtime_values
...
def _prepare_flat_vector_execution(
    ...
    if mode == "vmap":
        return dataclasses.replace(
            execution,
            flat_parameter_vector_batch=build_flat_vector_batch(execution),
        )
...
def per_example_sliced_executions(
    ...
    """
    batch, batch_in_dims = per_example_batch_in_dims(
        execution.batch,
        label,
    )
...
def _per_example_gradient_matrix_without_manual_batch(
    ...
    return per_example_gradient_matrix_from_builders(
        execution,
